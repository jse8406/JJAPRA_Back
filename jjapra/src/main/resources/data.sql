INSERT INTO member (username, password, name, email, phone_num, role) VALUES ('test1', 'test1','test1','test1@naver.com','010123456478', 'ADMIN');
INSERT INTO member (username, password, name, email, phone_num, role) VALUES ('test2', 'test2','test2','test2@naver.com','010123456479', 'USER');
INSERT INTO issue (title, description,writer) values ('제목', '내용','test1');
INSERT INTO issue (title, description, writer) values ('제목2','내용2','test2');
INSERT INTO issue (title, description, writer) values ('제목3','내용3','test1');
INSERT INTO project (title, description) values ('DDing', 'hi');
INSERT INTO project (title, description) values ('Channy', 'hi');
INSERT INTO project (title, description) values ('Unsta9ram', 'hi');
INSERT INTO project (title, description) values ('Miging', 'hi');
INSERT INTO project (title, description) values ('Suzzang', 'hi');
